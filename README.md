...
A Laravel 13 app with a [Filament](https://filamentphp.com/) administration panel.
...
``docker exec train-app pnpm run dev``

or if you want to watch for changes.

``docker exec train-app pnpm run watch``
...
To authenticate with Shibboleth, set `SHIB_HOSTNAME` and `SHIB_CONTACT` in `docker/shibboleth.env`, and set `SHIBBOLETH_AUTH_ENABLED` to `true` in `site/.env`. If `SHIBBOLETH_AUTH_ENABLED` is set to `false` you will only be able to use the Filament authentication.
...
Use a reverse proxy configuration to map the url to port `8686`, which is served by the Shibboleth proxy. SWITCHaai requires HTTPS, so the reverse proxy must terminate TLS.
